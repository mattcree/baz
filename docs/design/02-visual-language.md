...
> |---|---|
> | §2.7 — density placed in **Settings → Appearance** | **Superseded.** The three named steps survive as data; the step persists as state, and Settings is never the answer to a view question (ADR-0017 §1.3). The gesture-only phase ended with ADR-0028: the visible control is three detent marks at the foot of the index rail's lane, and `Ctrl+-` / `Ctrl+=` / `Ctrl+scroll` accelerate them |

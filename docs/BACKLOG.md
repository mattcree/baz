...
> known gaps and promises. Updated 2026-08-14.
...
| Ask | State | Where |
|---|---|---|
| *"the sticky bar when scrolling through artists etc. has too much padding on the left so it is misaligned"* | **alignment defect recorded; queued as item 22** | Re-measure the shared sticky section header against the content column it labels. Its visible text/ink edge—not merely its container box—must follow the same left alignment law as the artist, album or other grouped content below it, without adding a one-page offset. Audit every wall that reuses the sticky-header component, all density steps and responsive widths, including the transition from an ordinary section heading into its stuck state; sticking must not introduce a horizontal jump. Preserve the intended full-width background, clipping and hit geometry while removing only the excess inset. Acceptance uses same-size captures and measured edges for Artists plus every other consumer in both resting and stuck states. |
| *"when I scroll the album images, I am seeing them actually over the top of the top and bottom bar, as if their 'z index' is wrong"* / *"after a reset it's ok... maybe worth investigating how this might happen"* | **intermittent rendering defect recorded; queued as item 21** | Scrolling artwork must remain clipped to the content viewport between Baz's resident app bar and bottom transport. A reset cleared the observed fault, so a clean state is not evidence that it is fixed: reproduce the transition into the bad state and identify exactly what kind of reset repairs it and which layout, clip, compositor or cached widget state that action rebuilds. Capture the preceding navigation, resize, density/display change, long scroll, artwork load/cache churn and renderer/chrome state before naming the cause; it may be invalidation or clipping/layout order rather than a web-style z-index fault. Audit every scrolling surface that can draw album or playlist sleeves. Off-viewport artwork must neither paint over nor receive pointer interaction through either bar; fix the shared viewport/chrome composition rather than masking individual tiles with padding. Acceptance needs a repeatable bad-state sequence, captures at both boundaries, and a regression assertion that content clipping and resident chrome ordering survive the triggering state transition across supported renderers and native/borderless chrome states. |
...
| *"lets make sure we tackle the home page after critical usability stuff is done -- lets get the vibe playlists etc. based either on lightweight nlp/machine learning options etc. etc."* / *"it should be genuinely impressive and a unique selling point for a local music player app without internet connectivity"* | **shipped — local Vibe; semantic research deferred** | The owner accepted the current local Vibe feature after using it: opt-in cancellable analysis over Baz's hardened decoder, a separate stale-file-aware cache, Calm/Warm/Focus/Bright/Drive controls, explicit sounding-track anchoring, continuity sequencing and artist/album diversity. The normal Baz build includes it; `--no-default-features` is only an internal dependency-boundary check, not another distributed product. The semantic LAION/DCLAP harness, reproducible Apache-2.0 LAION export and four-system private blind ballot remain research tools. A semantic model may later be integrated only with an audited runtime/model payload and demonstrated listener value—never as a surprise download or an empty build flag. No cloud, account, remote model, hidden pool, engagement optimisation or automatic regeneration. |

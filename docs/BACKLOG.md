...
> known gaps and promises. Updated 2026-08-12.
...
| *"on Windows, the user could see a visible command line window opened alongside the app"* | **Windows release-packaging defect recorded for later prioritisation** | A normal Windows launch must create only the baz GUI window, never a companion console. The binary currently has no `windows_subsystem` declaration, so Windows treats it as a console application. Mark packaged/release Windows builds for the GUI subsystem at the crate entry point while retaining a console for debug/developer builds, where stderr diagnostics are useful. Do not hide failures with the console: anything a listener must know belongs in the canonical health/event surface, and release diagnostics that remain necessary need an intentional file or Windows logging path. Acceptance is a launch of the actual packaged `.exe` from Explorer and the Start menu with no console flash or persistent terminal, plus a debug launch that still exposes diagnostics. |
...
| *"the Playlists page does not have the rail on the right and seems to be different from the Library? it should not be significantly different"* | **collection-surface convergence recorded for later prioritisation** | Interpreted as the root `Place::Playlists` collection, not one playlist's detail page. It should use the Library wall's collection scaffold—including the right-hand index rail—rather than merely borrowing `shelf::Grid` inside a separate page implementation. Playlist-specific facts remain legitimate: collage tiles, playlist counts, and `A–Z` / `Date created` / `Played` ordering. The viewport, hang/gutters, virtualization, scrollbar relationship, density behavior, hover/selection grammar and rail anatomy should be shared so the two collection pages cannot drift. This resolves ADR-0024's earlier deferral of “a playlist's rail sorting” in the owner's chosen direction. The later design still has to name what the rail indexes under non-alphabetical orders: derive meaningful date/recency buckets from the active order, or keep an alphabetical rail only when `A–Z` is active; do not draw letters that jump unpredictably through a date-sorted wall. Acceptance starts with same-size captures of Library and Playlists and a code boundary where one collection scaffold receives different items/group projections rather than two lookalike scroll/rail implementations. |
...
| *"ideally we could have a back and forward functionality via back and forward nav arrows in the top left similar to Spotify"* | **recorded for later prioritisation; requires a navigation-model decision** | Browser-style **place** history, distinct from Previous/Next track: top-left Back and Forward arrows walk the places and subjects the listener actually visited. This deliberately reopens `place.rs`'s written decision that places replace one another, there is no history stack, and Back always means Library; it also amends ADR-0040's closed app-bar tenancy and zone 1, currently reserved for baz's identity. The expected browser rules are the starting point: a new navigation after going Back clears the forward branch; repeated navigation to the current place adds nothing; unavailable subjects fall back safely rather than trapping the listener; disabled arrows remain stable and visibly unavailable. The later design must decide whether history captures only `Place` identities or also meaningful local state such as the Library query/scroll, whether Esc remains “peel this layer then return to Library” or becomes history Back, and whether breadcrumbs/resident destinations push entries. Consider conventional accelerators (`Alt`+Left/Right and mouse Back/Forward) with the visible arrows, but the owner's ask is the on-screen controls. |
| *"when folders are offline we show a message in two places. the little status log is where I'd prefer is the source of truth"* / *"if there is an important status i.e. red status, can we make it pulse a bit and be noticeable?"* / *"for any event we come up with, can we have the concept of 'retry' ... either that or ... retries occasionally ... maybe exponential backoff"* / *"our status indicator should just go up into the top bar as a notification bell"* | **recorded for later prioritisation; destination revised to the app bar** | The health/event history remains the canonical surface for operational problems, rather than repeating the same offline-folder fact in Settings and status, but its one resident door is now a notification bell in the app bar—not the current bottom-bar dot. The bell belongs in the app-wide zone beside Settings, is present in every place in a fixed hit box, and opens the event history from that top-bar anchor; remove the duplicate bottom-bar indicator so there is one status entry point. Its quiet/read state must remain quiet, while a new important unresolved error may briefly pulse or carry a restrained severity/unread badge without becoming permanent motion. Model recovery with the event: either an explicit `Retry` action when the operation is safely repeatable, automatic retry with bounded exponential backoff, or both. A retry must update/resolve the existing condition rather than append endless duplicate failures, and success must visibly return the bell and summary to good. The current five-minute periodic library rescan already retries unavailable roots coarsely; later design must decide whether that is sufficient, should be surfaced as retry state, or needs a quicker targeted retry. Open questions intentionally retained: exact bell/badge states and acknowledgement semantics, panel alignment and narrow-window tenancy with the app-bar controls, which events are retryable, manual versus automatic policy, backoff/cap/reset rules, and what Settings retains once status owns runtime health. |
...
| *"lets make sure we tackle the home page after critical usability stuff is done -- lets get the vibe playlists etc. based either on lightweight nlp/machine learning options etc. etc."* | **next feature-area direction after the critical-usability tranche; signal research unbuilt** | Home is the next product surface to tackle once the current critical usability work is complete, and vibe/prompt playlists are its substantive feature—not a cosmetic Home rearrangement. Preserve the model already designed in ADR-0024 and designs 08/09: generation happens only when a listener asks, produces an ordinary named/editable/saveable `.m3u8` with inert provenance, states its candidate pool, changes nothing later without another request, and never begins playback by itself. Research the smallest useful **local-first signal ladder** before choosing a dependency: (1) deterministic metadata/history filters and a small prompt grammar; (2) lightweight local NLP/classification or embeddings over the listener's own tags and request; (3) incremental local audio features such as tempo, energy and timbral similarity, reusing the shape—but not pretending to reuse the output—of the ReplayGain analysis pipeline. Compare result quality, binary/model size, CPU, memory, analysis time, licensing, cross-platform packaging, offline behavior and explainability against simple baselines; “machine learning” is an option only if it wins that comparison. Home should provide the request/vibe entry point, visible scope and progress, results/provenance, and the ordinary playlist it created; it must handle sparse or unanalysed libraries honestly and let the listener edit/regenerate/save without inventing a second playlist species. No cloud, account, remote model, hidden pool, engagement optimization or automatic regeneration. Later design must define the prompt vocabulary and/or vibe controls, diversity/artist repetition rules, cancellation/background analysis, index/version migration, evaluation corpus and what explanation is useful without exposing meaningless scores. This ordering does not move it ahead of critical usability or turn it into a beta blocker; it makes it the first major product-feature area after that tranche. |

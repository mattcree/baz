...
| Ask | State | Where |
|---|---|---|
| **Not an ask — found while verifying the above.** The notification bell draws as a solid disc | **backlogged 2026-08-14 — seen, not yet argued** | It was invisible until now because the sheet was handing `Bell` the *forward arrow*'s sprite (see the back/forward row below); with the index fixed, the real `BELL` outlines draw for the first time in the app bar and they are **a filled blob**. The glyph's own doc comment says *"a dome, clapper and short base at the shared icon stroke"* and the outlines are neither stroked nor clappered: one eleven-vertex solid silhouette from y 0.16 to 0.76 across x 0.22…0.78, which at [`ICON_PX`] 20 is a circle, plus a 1.6 px nub below it that does not survive minification. It is also against the sheet's own stated rule — *"the arrows are strokes, not filled triangles… one stroke weight is what makes the sheet one set"*. *Deliberately not fixed in the same pass: it is a new observation rather than one of his, and the standing rule is that those are triaged here first. Needs later: whether the bell is redrawn as a stroke at the set's 0.145 like `OPEN` and the window controls, or kept a silhouette and reshaped so it reads as a bell rather than a dot — and note the status dot already stacks on its bottom-right corner, so a stroked body has to leave that corner legible.* Frame: `docs/design/impl/backlog-pass-2026-08-14/`. |
...
| *"the right hand rail is acting strangely"* | **still open 2026-08-14 — the first suspect is eliminated, not confirmed** | **`RAIL_HIT` is not the rail's.** It belongs to the volume groove and the seek needle (`groove.rs`, `needle.rs`, `pointer.rs`); `spine.rs` never mentions it, nor `STEPPER_HIT`. And `spine.rs` and `rail.rs` are **untouched** by the control pass, as is every token they read — `INDEX_W`, `INDEX_CLEARANCE`, `INDEX_LANE_W`, `HANG`, `RAIL_PITCH`, `RAIL_LINE_H`, `MAGNIFY_MAX`, `MAGNIFY_REACH`, `MAGNIFY_SPREAD`, `SIZE_HEADING`, `LEADING_HEADING`. So the lens, the pitch and the hit band are all exactly what they were, and the earlier diagnosis here was wrong. **What did move is the lane the rail stands in**: `APP_BAR_H` 41 → 49 and `TOP_BAR_H` 49 → 57 take **16 px** off the body, and the rail's capacity is `floor((lane_h − 2 × MAGNIFY_SPREAD 45) / RAIL_PITCH 20)`, so the window height at which the full 27-letter alphabet still fits rises from about 801 to about **817**. Between those two heights the rail newly elides, and an eliding rail — letters coming and going, the lens landing on a boundary rather than a letter — is the shape a *"strangely"* report usually has. *Needs first, and only from him: what "strangely" is, and at what window height. If it is letters disappearing, the 16 px is the cause and the fix is the elision threshold; if it is the lens aiming off-target or the band catching presses meant for covers, nothing in the pass explains it and it is an older defect.* |

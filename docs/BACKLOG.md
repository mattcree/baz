...
| *"we need some sort of min height as well"* | **shipped** | there was none — `app.rs` passed `min_size` a height of literally `0.0`, so the window could be dragged shut to a strip of furniture with no collection under it, while the *width* had been floored since the strip's audit. One had been thought about and the other had not. `theme::WINDOW_FLOOR_H` is **derived, not chosen**: the tallest chrome (the two-line strip, the bar, its hairline, the needle) plus **one row of the tightest wall**. One row is the whole of the claim — below it the window is two bars agreeing about a collection you cannot see; it is not a claim that one row is comfortable |
...
| *"maybe we could put the search in the top bar?"* | **his question, priced, not built** | **the case for.** His own admission rule for this bar, verbatim — *"adding controls that apply to all windows makes sense in the top bar"* — and search applies in every place more cleanly than the display options do: type-anywhere (ADR-0017 §1.2) means any printable key opens a query from anywhere, whereas the marks are absent on four of the seven places. It would also end a real duplication: the well has **two** homes today, the lane above `SIDEBAR_FLOOR` 1000 and the Library strip below it (*"one home per regime, never two"*, ADR-0030's second amendment §4), and the app bar is the only surface that is resident at every width *and* in every place. **The case against.** ADR-0036 gave the well one meaning and one home, and it is in the lane on his own earlier instruction — *"the search should really be in the sidebar"*. ADR-0030's second amendment §2 records what that cured: the strip and the lane both carried the frame's identity, so the eye had two places to start. A well at the top of the window revives exactly that, against a *third* surface this time. ADR-0040 §1's closed tenancy names the well as the lane's, in a test. **The cost, measured.** The field fits and its readout does not: the bar has 304 px of slack at the window's floor and the well is `SIDEBAR_MEASURE` 232 + a `GAP_LG` seam, leaving 56 — but the band is one `TRANSPORT_HIT` tall, which is exactly the well's height, and the always-drawn counts line under it (`25 albums · 206 tracks` at rest, `12 of 25 albums` while narrowing) has nowhere to go. It was put under the field precisely because the lane's 232 could not hold it inline. So the honest options are: drop the counts, give the bar a second line (which is the app bar becoming the strip it replaced), or move the counts to the strip below — and the third re-splits the frame's identity across two surfaces, which is the thing being cured. **Recommendation: no**, and it is close. The rule admits it; the composition is what refuses it, and the refusal is his own from three days ago. If he wants it anyway, the cheapest honest shape is the *collapsed* well — the magnifier alone in zone 3, opening the lane onto the caret exactly as `SIDEBAR_RAIL_W`'s does today — which costs 32 px, keeps one home for the field and one place for the counts, and adds a second road to a query that already has four. **His call.** ADR-0040's amendment §3 |
| *"until we have no window chrome, remove the window controls..."* | **shipped** | with the platform's title bar still above baz's own band, minimise/maximise/close were drawn **twice**, and one pair did nothing the other did not. They are **conditional, not removed**: `app::owns_chrome()` is the single answer that both turns `decorations` off and tells the bar whether to draw them, so the day `BAZ_BORDERLESS` becomes the default they appear with no second edit and there is no build that owns its chrome and cannot be closed. The slot is **not** held open — an empty reservation for a control that cannot exist in this state is the present-and-inert failure the bar's own admission rule refuses, and `owns_chrome()` is settled once per process, so there is no frame in which they arrive and nothing can be *seen* to move. (This line used to end *"and nothing to its left moves, because they sit at the trailing edge"*, which was false by 120 px — everything to their left moves when they appear, and that is the price of not holding the slot. It was false by a further 16 in the state that ships, because the row spent a seam on the placeholder standing in for them; see the cog row above.) The band keeps its drag and its double-press either way: those *add* a way to move a window that already had one, where a second close button subtracts clarity |
| *"I think ideally we could ensure our playlist view in the now playing and the playlist view/album view are the same thing. the only thing that changes in now playing is that we don't see file details etc. -- that is more like a album exploration type data"* | **building** | the **third** copy of one list. `views/page.rs` merged the record's and the playlist's today, on his earlier *"can we reuse the basic layout and view of the playlist for the album view"*; the run column in `Now playing` was outside that merge and is now pointed at. He has also named what may differ — the `DETAILS` block is exploration data and has no business beside a run you are listening to. Anything else that differs must be justified or removed |
| *"also please make sure the layout of the now playing makes sense on wider screens"* | **building** | a re-report of doc 12 step A4, already a beta blocker: at 2560 with the run standing, **~700 px of empty field** sits between the sleeve and the run column, because the record column hangs left and `RUN_MEASURE` stays 440. His first telling named **both** edges — *"the playlist hugs right and the art hugs left"* — so the record column's own alignment is checked too, since widening the run would otherwise hide a second fault rather than fix it |
...
| *"the seek bar at the bottom should have a toggle indicating for song or for whole playlist"* | **logged, undesigned** | the bar can measure the track or the run; both are true readouts and the listener says which |
...
| *"the artist page should have its own 'all songs' playlist I think"* | **logged, undesigned** | the same `implicit::ImplicitList` that gives the library its `All songs`, scoped to one artist — so the machinery exists and the questions are its ordering, where the tile sits on the page, and what it is called |
...
| *"how the search works when we're not on the library needs to be decided. should it just pop to the library view when you start typing? or should it search whatever page you are on?"* | **Decided: it pops to the Library, which is what already shipped** — and the field now *says* so, reading `Search library` rather than `Search` in every place. Contextual search refused on one hard constraint: type-anywhere is a promise about the collection, and a scoped well would revoke it on exactly the pages a scope applies to (ADR-0036 §3). The playlist filter the question is really about is costed in the sections below as a *second control*, which is his to call |

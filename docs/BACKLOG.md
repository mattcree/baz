...
| Ask | State | Where |
|---|---|---|
| *"when I scroll the album images, I am seeing them actually over the top of the top and bottom bar, as if their 'z index' is wrong"* | **rendering defect recorded; queued as item 21** | Scrolling artwork must remain clipped to the content viewport between Baz's resident app bar and bottom transport. Reproduce before naming the cause: it may be clipping/layout order rather than a web-style z-index fault. Audit every scrolling surface that can draw album or playlist sleeves, including fast/inertial scroll and each density/window-size breakpoint. Off-viewport artwork must neither paint over nor receive pointer interaction through either bar; fix the shared viewport/chrome composition rather than masking individual tiles with padding. Acceptance needs captures at both boundaries while scrolling, plus a regression assertion that content clipping and resident chrome ordering survive the supported renderers and native/borderless chrome states. |

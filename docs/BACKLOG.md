...
| Ask | State | Where |
|---|---|---|
| *"lets go with a simpler feature for now, skip this one"* (smart shuffle) | **deferred 2026-08-20** | No Flow traversal was implemented. The investigation remains at `docs/design/27-smart-shuffle.md`: it separates reordering a finite queue from an endless, steerable local-radio feature, which need distinct product decisions. Ordinary Shuffle is unchanged. |
| *"as a general rule we will likely not include too many features that reach out to the internet. in fact, ideally none."* | **policy recorded 2026-08-20** | baz remains offline-first. The optional update check is its sole runtime-network exception, not a precedent: new product features must remain local unless the owner specifically authorises a named exception. A listener can turn that check off in Settings; the Flatpak has no network permission. [ADR-0043](adr/0043-installing-and-updating.md) carries the binding amendment. |
| *"the colours on the now playing need to be part of the theme otherwise it looks weird"* | **shipped 2026-08-19** | Half of it already was. The **wash** reconciles: `Field::colors` starts at the room's own `wall` lightness and climbs by `rise`, in the direction that room's ladder climbs — so a light room's field descends. The **ink** did not: a flat `INK_L` 0.72 in every room, which is a bright colour over Closing Time's near-black wall and a pale one over Plaster's, where it needs to be dark.<br><br>The ink keeps its **distance** rather than its value now. `INK_RISE` 0.562 is `0.72 − lightness(CLOSING_TIME.wall)` — the gap the shipped ink already had from the room it was chosen in — and every room gets that gap in its own direction. A new room needs no edit to be drawn correctly, which is the same bargain `rise` strikes.<br><br>Floored at L 0.45, and the floor is about **colour** rather than contrast: a light room would put the ink at 0.36 by the rise alone, and the sRGB gamut is narrow down there for warm hues — a dark yellow is an olive, and the tightest hue's ceiling falls to 0.045, which is grey. 0.45 keeps it above 0.076 while still standing 0.47 clear of a plaster wall.<br><br>**And sweeping the rooms found a real bug in `safe_chroma`, shipped an hour earlier.** Its binary search assumed `clips` was monotone in chroma. It is not: past the gamut boundary `from_oklch` clamps, and a clamped channel comes back *inside* range as chroma keeps rising — at L 0.45, hue 264, red reads 0.0004 at c 0.28, 0.0000 at 0.29, and 0.0084 again at 0.311. The bisection landed in that dip and returned an out-of-gamut chroma with a straight face. It walks to the **first** crossing now and bisects only the step it is inside. A search that had only ever run at one lightness could not have shown this; the room sweep did on its first run. |
| *"make the left hand bar go to the top and include the back and forward icons in it. the search then becomes part of the main area's top bar"* | **shipped 2026-08-19** | The frame was a `column!` of bar over a `row!` of lane and body. It is a `row!` of lane and everything-else now, and the everything-else is a `column!` of bar over body — so the lane runs the window's full height and the bar belongs to the **body**.<br><br>The app's mark and the history pair moved into the lane's head. That cost nothing to align: the mark's centre was already on `SIDEBAR_HEAD_GLYPH_X`, the lane's own glyph column — it had been aligned *to* the lane from the bar since 2026-08-14, so it lands exactly where it already looked like it was. Their guards moved with them into `views::lane`.<br><br>**Collapsed, the head stacks.** The lane is one glyph wide there and a mark with two arrows beside it is three, so the pair sits under the mark rather than being clipped off the edge. Losing them would have left `Alt+←`/`Alt+→` as the only route to history, which the keyboard rule forbids: every binding is a press some visible control also makes.<br><br>**Three budget guards caught the move**, and one found a latent hole. `APP_BAR_LINE` is 152 px shorter (870 → 718) and `TENANTS` lost two entries; the zone diagram went from seven named zones to five. That dropped `WINDOW_FLOOR_W` — and `bar_title_lane_w(WINDOW_FLOOR_W)` went to **zero**, because the *bottom* bar's width had never been a term in the floor. It was covered by the app bar being wider, which is coincidence rather than derivation. `BOTTOM_BAR_LINE` is a floor term now: a floor that holds one bar by accident is not a floor. |
| *"in the 'full screen' can you make sure the top bar is transparent essentially… all the same icons as the normal one, but the visualiser and background colour etc should be showing where it is currently black"* | **shipped 2026-08-19** | The bar had been transparent since the field arrived behind it, and read **black anyway** — because the field and the spectrum lived *inside* the page, and the page is the window less the bar and the lane. There was nothing under the glass but the window's own ground.<br><br>**The obvious fix was wrong and got reverted.** Stacking the bar *over* the page gives the page the full height, and it also moves every place up under the bar: the lane has no margin of its own and ran off the top of the window. The layers that want to be behind the bar are the field and the spectrum, so it is **those** that leave the page — they are the window's backdrop now (`now_playing::backdrop`), stacked behind the bar, the lane and the body alike. The lane keeps its own ground and stays readable over them.<br><br>**Chromeless carries the ordinary bar** rather than the reduced strip it had. The ask is for the lane and the bottom band to go, not for the doors to — an immersive mode you cannot change the visualiser from is a mode you leave in order to change the visualiser. `app_bar::chromeless` is deleted, and the cluster-seam guard went back to three. |
| *"the colours for the visualisations just aren't very interesting"* / the earlier *"colours of the background and visualisations aren't very striking or dynamic"* | **shipped 2026-08-19** | Measurable rather than a matter of taste, once the right thing was measured. `INK_CHROMA` was a **single** chroma that had to be safe at every hue, so it was pinned by the *worst* one — and every record paid the blue's price whatever colour it actually was.<br><br>The ceiling at the shipped lightness, swept: **0.1245** at hue 210 (a cyan-blue, the tightest corner of sRGB up there) to **0.2854** at hue 330. A **2.3× spread** behind one constant of 0.11.<br><br>`safe_chroma` finds each hue's own ceiling by binary search — twenty steps, three per frame, against a visualiser already rasterising a field — with a margin so the answer sits just inside the edge it found. A magenta record draws at more than twice the chroma it used to, a blue one is unchanged, and nothing clips: the same guarantee, charged fairly. The test swept one constant before; it sweeps all 3600 hues now and asserts both that none clips **and** that the mechanism is buying something (the widest hue must beat the old flat 0.11 comfortably), so it cannot quietly degrade back into one number.<br><br>Lightness was checked first and was not the answer: the hue-independent ceiling *peaks* near the shipped `INK_L` 0.72 (0.1224) and falls either side, so there was nothing to win by moving it. |
| **Not an ask — found while running the checks.** The disk was full and it was mine | **cleared 2026-08-19** | `/var/home` sat at **471 G of 475 G**, with `target/` at **145 G** — 124 G of it `target/debug`, accumulated test binaries from a long session of `cargo test --workspace` runs. `/tmp` (a 24 G **tmpfs**, so RAM) held 17 G of screenshots.<br><br>**It was also the real cause of the "hanging" tests.** The `playback` integration test had been diagnosed as hanging on concurrency — four stuck binaries, the oldest five hours old — and the overlapping runs were real and did starve each other. But run singly it finishes in **5.6 s**, and underneath was a full disk: that test shells out to `ffmpeg` to build audio fixtures, and those writes had nowhere to go. The concurrency was a symptom.<br><br>Found by accident: `pwd` failed with *Disk quota exceeded* in the middle of an unrelated `grep`. 140 G freed, all of it regenerable build output. *Worth a periodic `cargo clean` in any long session — nothing prunes `target/debug` on its own.* |
| *"when it rotates to show the back, instead just show the frontal view… make it look like it rotates to show the front again and again"* / *"the text on the edge of the box… on the left hand side it's actually back to front"* | **shipped 2026-08-19** | Both flat faces draw the **front** now. The rear inlay was a blur of the cover with the edition's track list printed over it, and half of every turn was spent on it — while the front is the reason there is a sleeve on this page at all.<br><br>**It also deleted real work per record**: `generated_rear` rendered that image on every record change, and `rear_insert` resolved the edition and cloned every title to feed it. Nothing sampled the result. Removed rather than left behind an `#[expect(dead_code)]` — git has it if a back is ever wanted.<br><br>**Two wrong turns on the way, both worth recording.** I assumed the back face needed its UVs flipped and added one; it made things worse, and only tinting a face red and photographing it showed that my model of which face was which was wrong. `POS` already winds the rear corners *as seen from behind*, so the front art lands un-mirrored with no help.<br><br>Same shape of error on the spine. The old code flipped one edge; I moved the flip to the other edge and the fault moved with it, which is what diagnosed it. The two spines are wound in **opposite world directions** and each is seen from the opposite side — those reversals cancel, so neither needs flipping and the conditional was a third reversal. Verified across a full turn at both phases. |
| *"it looks like we have two gradients going on… can we instead just have one"* / *"can we make the album rotation half the speed"* | **shipped 2026-08-19** | They were two: the scrim behind the title, and the source band's own. The band draws **no ground of its own** now — the page's single fade reaches solid black exactly where the band sits, so the band is the bottom inch of that gradient rather than a second one starting where the first stopped. What is left on the band is press feedback, which it still owes a pointer that is over it.<br><br>The stack moved into its own `stage` function on the way, which also put `now_playing::view` back under its line budget — clippy caught that, and extracting was the right answer rather than an allow.<br><br>`TURN` is 64 seconds rather than 32. `TICK` is unchanged, so the motion is as smooth as it was and simply covers half the angle. |
| *"the way it's rotating now isn't nice"* / *"the gradient looks a bit poor here… maybe just make it fade to black?"* | **shipped 2026-08-19** | **The rotation: I over-corrected, and the second complaint exposed the first diagnosis.** Solving for the lens per frame stopped the clipping by holding the case one size through the whole turn — and that swell toward the camera *is* the rotation, the only cue that the thing turning has a side rather than being a picture on a spinning card.<br><br>The constant was never the problem. **The box was.** The case had 0.79 of a box `object_region` had shrunk by a third; with the layout corrected, 0.79 of the region is a large album and the perspective can be left alone. So it is fixed again — but *derived* from the angle where the projection is widest (`2 · fill · (2.55 − 0.5)` = 4.02) rather than guessed, and the test asserts a **range**: it must fit at every angle, and must **not** be the same size at all of them. A future fix cannot flatten it again.<br><br>**The band fades to black**, and the reason is worth keeping: the field behind it is already the record's colour, so tinting the band from the same well made one hue fade into a near-identical hue — close enough to read as a printing error. Black is not a colour competing with the field; it is the field running out. The tint helper is deleted rather than left unused. |
| *"when the play button is pressed for a playlist it does not go to the now playing screen"* | **shipped 2026-08-19** | `play_playlist` carried its own copy of `start_and_show`'s four lines minus the one that matters, which is the whole argument for the shared tail existing — so the duplicate was deleted rather than a `go` added beside it.<br><br>**Two more were the same**: `play_favourites` with no lead, and the `All songs` tail. All four whole-list Plays now share one path, confirmation boundary included — the place changes when a track actually *starts*, not when the channel accepts the command.<br><br>**A row press deliberately does not navigate**, and `play_track` on a record's page already drew that line: pressing `Play` on a list is a decision to listen to it, while pressing one of its rows is a decision made *while browsing*, and taking the reader away from the list they are reading answers a question they did not ask.<br><br>The guard that existed asked whether each gesture went through the **arranger**, which `play_playlist` satisfied while forgetting the place — so the new one asks the other question, and asserts the row presses *stay*. Confirmed by reintroducing the defect and watching it name `play_playlist`. Split into its own test rather than silenced with an allow when the pair went past its line budget. |
| *"can we get it set up to be installed not just built — as in when it shows up it has a proper icon set etc."* / *"is the icon being set on macOS?"* | **shipped 2026-08-19; macOS confirmed** | **macOS: yes**, and verified by building a bundle rather than by reading the source. `bundle.sh` builds the `.icns` with Apple's own `iconutil` from the committed iconset, `Info.plist` carries `CFBundleIconFile`, and it lands at `Contents/Resources/`. The committed fallback icns is well-formed (TOC, all ten sizes) and is used only by CI's Linux shape-check — never by anything a person downloads. `bundle.sh` already records why Apple's tool builds the shipped one: a hand-written icns once made macOS draw the generic icon over it.<br><br>**Linux was the gap.** The archive already carried the desktop entry, the AppStream metadata and the whole hicolor ladder — and `INSTALL.md` then handed the reader five `install -Dm644` lines. That is an application plus a list of chores.<br><br>`install.sh` and `uninstall.sh` now travel in the Linux archive. Per-user into `~/.local` by default and needing no privilege, `--system` for `/usr/local`, `--prefix` for anywhere. It rewrites the entry's `Exec=` to the binary it actually installed — a desktop's `PATH` is not the shell's — and keeps a **manifest** of every file it wrote so the uninstall removes exactly those and refuses rather than guessing when the manifest is absent. It does **not** edit `PATH`: changing a login profile from an installer is a surprise found months later in a file you did not write, so it says so and prints the line.<br><br>CI runs the whole round trip on every push — install into a scratch `HOME`, validate the rewritten entry, uninstall, assert nothing of ours survives, and assert a second uninstall fails. Same reason the macOS bundle is assembled there: an install path nobody exercises until a release is downloaded is one nobody reviews.<br><br>*Still open: a one-file install. The Flatpak manifest exists and CI validates it, but no `.flatpak` bundle is built.* |
| *"the 3d cd case is being cut off at the top and bottom when it spins"* | **shipped 2026-08-19 — a regression, and mine** | Raising `lens` 4.0 → 4.75 to fill the box ignored what the yaw does to perspective. The projected half-extent is `lens · bounds / (4 · view_z)`, and `view_z` bottoms out at `2.55 − (0.5·|sin θ| + D·|cos θ|)` = **2.05** edge-on — so 4.75 projects **1.16×** the box at 90°. 4.0 gave 0.98 there, which is exactly why it was 4.0: it was sized for the worst angle and therefore drew the case at 0.78 of its box at rest.<br><br>**A fixed lens cannot hold a constant size through a rotation**, and both readings were of that one mistake. The CPU solves `lens = 2 · fill · view_z_min(yaw)` per frame instead: 4.83 at rest, 3.94 edge-on, and the case fills 0.94 of its box at *every* angle — no clipping, and no breathing either, which was the other thing wrong with it. `the_lens_never_lets_the_case_leave_its_box` sweeps 721 angles; confirmed by restoring 4.75 and watching it name yaw 0.29. |
| *"there is a black gap at the right hand side"* / *"the visualisation just isn't being drawn at the full size of the window when in full screen"* | **shipped 2026-08-19** | One cause: `body_width()` subtracted the returns lane and `body_height()` the bottom bar, while chromeless had hidden both. Every place that sizes itself against the body — the field, the spectrum, the record's own column — was drawn a lane short of the glass. Both now ask whether the frame is standing.<br><br>Also in this pass: the chromeless strip uses `theme::app_bar_pad()` so its marks sit on the framed bar's own verticals (*"make sure the padding is the same"*); the source band is taller with its fade starting higher and its tint dropped 0.22 → 0.12 (*"more muted, yet still based on the album"*); and the album stopped sitting top-right at narrow widths, which was `align_y(Center)` on a **shrink-height** container — there is no spare height to centre within, so the row placed it at the top. It only showed when the object was width-bound and therefore shorter than the stage.<br><br>*Still open from the same batch: the window controls fading in on approach. `near` is threaded and ignored — doing it honestly needs an opacity factor down through each mark's own image, because iced has no opacity wrapper for a container, and the conditional that would be quicker moves every widget under it on each approach.* |
| *"'keep' is not sensible — Save is fine"* / *"when we edit an existing custom preset we should give the option to save or reset"* | **shipped 2026-08-18** | `Keep` was mine and it was worse than the thing it replaced; the word is `Save`, and a test refuses `Keep` coming back by copy-paste.<br><br>**The second half needed a selection, not a match.** The slot had been asking *are these ten numbers one of my curves*, which is a different question from *am I editing one of my curves* — and the moment a listener nudges a band the first answers no, taking with it the relationship the panel should be offering to save or reset. So the app tracks which saved curve is on the faders, seeded at launch from the config: without that, opening baz onto a saved curve and moving one band offered `Save` — a new name for a curve that already had one — instead of `Save`/`Reset` over the one it plainly is. The relationship has to survive the first nudge, which is exactly why it cannot be re-derived from the bands each frame.<br><br>Four states: an **offered** curve gets nothing (saving a copy of `More bass` under another name is not a thing to offer, and deleting it is not yours to do); a curve that is **nobody's yet** gets `Save`, which asks for a name; **yours, untouched** gets `Forget`; **yours, changed** gets `Save` over it under its own name and `Reset` back to what that name means. The picker keeps the curve's name through the edit rather than falling back to `Custom` — reset to *what*, if the panel has stopped admitting which curve this is? |
| *"the lines of the graphic EQ do not seem to be spread out enough"* | **shipped 2026-08-18** | They were on `GAP_XS` — a 4 px seam between 32 px columns, which is the rhythm of a **detent run** (one control with several states, touching) rather than of ten separate controls. Ten faders that nearly touch read as a fence. `GAP_MD` now: pitch 36 → 44.<br><br>The constant lives in `fader.rs` beside `HIT_W` rather than at the row that draws it, because the pair is one fact — the **pitch** — and `crate::response` derives the drawn curve's frequency axis from it. Changing the gap at the row alone would slide the curve off the handles it is meant to pass through; `the_curve_peaks_under_the_fader_that_raised_it` would catch it, but only after somebody had worked out why. |
| *"the album art is still too small. it should be taking up most of the area of the now playing"* | **shipped 2026-08-18 — the third telling** | Two goes at the number had already failed, so this time the two things that were *multiplying* got measured.<br><br>**The case fills 0.78 of its own box.** `jewel_case.wgsl` projects the half-width as `0.5 · lens / view_z`, and `lens 4.0` over `view_z 2.55` is 0.784 at rest and less when the case is turned. A 660 px box therefore drew about 370 px of album, and every argument about the box size had been arguing about the wrong 40%. `lens` is 4.75, which is 0.93 — short of filling the box because the near corner of a *turned* case projects wider than the flat front, and the rest is margin so a rotation can never clip.<br><br>**And the height term subtracted something that is not above it.** `object_region` read `height − footer − padding − gap − MARQUEE_BLOCK`, as though the object sat on top of the marquee. It does not: the marquee hangs from the bottom-**left** and the object stands to its right, so they share the vertical rather than dividing it. That was another 130 px. The stage is a `row!` now — beside, not above — and the **width** carries the relationship instead: the object takes what is left after the title's floor, so a narrow window shrinks the sleeve rather than crushing the title. A cover has a smallest useful size and a line of type has a smallest useful measure, and the type's is the harder floor.<br><br>Visible case at 1920×1080: **215 px → 530 px**.<br><br>Two documented properties had to be restated rather than quietly broken. *The measure depends on the window and nothing else* is kept by reserving the object's **column** rather than measuring the drawn cover — otherwise the title would rewrap every time a cover arrived at a different resolution or the foreground mode was cycled. And *wider windows give the title more room* became *never less*: between 760 and 1280 every extra pixel now goes to the sleeve, which is the ask expressed as an ordering. |
| *"we should still keep the window controls when we go into the 'full screen' mode"* | **shipped 2026-08-18** | The first cut of chromeless dropped the whole app bar, and my answer to the missing close button was a lone mark drawn on the Now playing page. That solved the wrong half. The question was never *how do I get back*; it is that on the platforms where baz draws its own title bar, hiding that bar takes minimise, maximise and close with it — and a window with no close button is not a mode, it is a trap.<br><br>So the strip stays and **empties** instead: a shorter bar with no ground of its own, carrying the window's own furniture plus the toggle that brought you there, which keeps the way out where the way in was. `now_playing::way_back` is deleted.<br><br>Two source-scanning guards failed on it, both correctly. `the_window_buttons_are_conditional_on_owning_the_chrome` counted call sites and asserted **one**; there are two now, so it checks the actual claim — every site is inside the condition — rather than a proxy for it. The cluster-seam count went 3 → 4. |
| *"make the area where the source is linked a gradient that fades from the top (more transparent) to bottom (solid)"* / *"make the source area on now playing related to the 'colours' of the track"* | **shipped 2026-08-18** | One change for both: a vertical wash, clear at the top edge so the field and the sleeve read through it, solid where the band meets the window. The tint is `field::Field::inks()`'s third ink at 0.22 over the room's own step — the same well the artist line above it draws from, so a record's page is one colour family rather than a coloured line on a grey band. A low weight on purpose: the band sits directly under a 64 px title whose contrast against it has to hold whatever hue the record happens to have. A record with no field falls back to the plain step. |
| *"make room is a poor term — just make it 'auto gain' as a checkbox. and change 'shape the sound' to EQ Enabled or something as a checkbox"* | **shipped 2026-08-18** | Right about the words and right about something bigger.<br><br>**The shape was wrong, not only the label.** This control had been renamed twice already — `Suggest a pre-amp`, which named the machinery, then `Make room`, which named the situation — and both were **buttons**. Pressing something once to fix headroom you are about to change again by dragging the next band is a chore, not a control; worse, a listener who pressed it and then boosted one more band was silently back where they started with nothing on screen to say so. As a standing mode it simply holds: the pre-amp is recomputed on the same frame as the band, so the fader tracks the drag and the sound never passes through a moment of being too loud on its way to a curve that fits.<br><br>**Taking hold of the pre-amp turns it off**, visibly. That is the honest resolution when the listener and a mode both want the same control — the mode yields and the checkbox unticks where they can see it — rather than a fader that moves and means nothing, or a mode that overwrites the next thing they do.<br><br>Auto gain never makes anything *louder*: `suggested_preamp` answers zero for a curve that only cuts, because turning a quiet recording up is a decision about taste and this is a decision about arithmetic. On by default, and **on for any config written before the key existed** — that document belongs to a listener who had no way to ask for it, and the safe reading of a boost with no stated headroom is that they did not mean to clip.<br><br>The label is `EQ enabled`. The line under the controls now has four states and a test holds them apart, because that line is the only place the panel explains itself and two states reading the same is the readout failing quietly. `read_equalizer` returns a struct rather than the four-tuple it had grown into — the tuple spread its call site over six lines and tipped `from_toml` one line past its budget, which is the lint doing its job. |
| *"we probably want a way to allow users to create and save EQ presets"* | **shipped 2026-08-18 — item 58's other half** | The offered curves answered *a few presets in a dropdown*; this answers the ask that argument never covered. The objection on record — *a list of named curves is somebody else's taste* — has nothing to say about **your own** curve, which is not somebody else's anything.<br><br>`Keep` when what is on the faders is not saved, `Forget` when it is one of yours, and **nothing at all** when it is one baz offers: there is no sense saving a copy of `More bass` under another name, and less in offering to delete something that is not yours to delete. Absent rather than disabled, which is this product's rule everywhere.<br><br>**A saved curve carries its own headroom**, unlike an offered one which derives it: a built-in states a situation and baz works out the room it needs, but a saved curve is *what you had set*, and someone who deliberately left themselves three decibels did not mean to save half of it. Saving over a name you already used **replaces** that curve — two rows reading `Kitchen` in one picker is a list that cannot be used, and *this is what Kitchen means now* is the ordinary reading.<br><br>In the config as `[[equalizer_preset]]` tables, one each, so a hand-editor can remove one without counting commas in a nested array. A malformed entry is skipped and the rest survive; values are clamped to what a fader can reach rather than refused; the list is capped at 24 and names at 32 **characters** — characters, because a byte limit would give a listener writing in Japanese a third of the name it gives one writing in English.<br><br>**Two faults found by driving it rather than by reading it.** The field opened *unfocused*, so baz's type-anywhere rule took the keystrokes to the app-bar search — a listener naming a curve would have watched their words filter their library. And the name came out `Kitchenspeakers`: one function was clipping *and trimming* on every keystroke, so the space was eaten the moment it was typed and the next letter landed against the last. Clipping and settling are two functions now, and the per-keystroke one is pinned as *not* trimming.<br><br>The panel also silently outgrew itself: the picker and `Keep` joined a controls row measured only by the **fader** row beneath it, and `Make room` wrapped onto a second line without complaint. `PANEL_W` is the wider of its two rows now, and `the_controls_row_fits_on_one_line` measures the strings the panel actually ships. |
| *"can you make the now playing album bigger though… it's just very small and cramped up into the corner"* | **shipped 2026-08-18 — the second telling, so the method changed and not the number** | The first draft took `room * 0.34`; told it was tiny, it became `0.58`; told again, the fault is the **method**. A share is a guess, and both guesses were of a fraction of something that was not the object's own region. The object *has* a region — the stage, less the source band, less the page's padding, less the gap to the marquee, less the marquee — and it was sitting there being ignored. At 1920×1080 the share drew 451 px into a region **660 px tall**, so a fifth of the room the composition had was empty for no reason anyone could state.<br><br>It is `min(region_w, region_h)` now, floored, bounded by the source — the same shape as `art_edge`, which the centred composition used before this page was rebuilt and which was right. `object_region` spells the subtractions out so they can be read against `view`'s own `column!`: the same page in two notations, and a term added to one has to be added to the other.<br><br>**And the guard was the cap checked against itself.** `edge <= LIST_MEASURE * 0.5` could only ever catch an object that was too *big* — a ceiling watching for a floor — which is why two undersized drafts passed it. It asserts against the region now, in both directions: the object fills what it is given, and the region still leaves the marquee and the footer standing.<br><br>Worth recording for the next screenshot: a jewel case is a **projected** object that yaws inside its box, so what reaches the glass is narrower than `edge` and breathes as it turns. A 215 px case had a 290 px box behind it. |
| *"we should consider adding a little toggle here which allows it to go into a sort of 'chromeless' mode which really shows off the now playing view"* | **shipped 2026-08-18** | The app bar, the lane and the bottom bar go, and Now playing is the window. Distinct from <kbd>F11</kbd> and **composes** with it: fullscreen asks the window manager for the screen and leaves baz's bars where they are; this is about what baz draws inside whatever window it has. Both at once is what *really shows off* is after.<br><br>**The obvious spelling would have been a bug.** Not `if !chromeless { column![bar, screen] }` — iced diffs the widget tree by position, so a column that loses its first child hands every widget under it a fresh state, and toggling the frame would have scrolled the run back to the top and dropped whatever the place was holding. It is the same fault `app.rs` already spends a long comment on for the floating layers (item 81, *"right clicking in a playlist seems to reset scroll position"*). The column keeps its arity; the slot empties.<br><br>**One way out that is always visible.** With the frame away there is no app bar and — where baz owns its chrome — no close button, so a listener who forgot which mark they pressed would be looking at a window they could not leave. <kbd>Esc</kbd> brings it back, and one quiet mark in the corner says so without words. Not persisted, deliberately: reopening days later into a frameless window from a press you have forgotten is not a preference, it is a trap.<br><br>**The mark joined the wrong family first.** Drawn at the application cluster's weight it measured 0.292 — correct beside the gear, and heavy enough beside the view's own marks (0.19–0.26) to have made *them* look thin, which is `EQUALIZER`'s lesson committed in the other direction. It is 0.246 now, and `a_mark_in_the_app_bar_carries_its_neighbours_weight` holds the bar's **two** families each to itself rather than flattening a deliberate difference in rank.<br><br>The lamp rule caught the last of it: the corner mark was amber until `the_lamp_is_named_only_where_playback_truth_is_drawn` refused it. With the frame away it would have been the brightest thing on screen and would have meant nothing. |
| *"can you also make sure the album is not tiny on the now playing screen!"* / *"the text alignment between the title text and the source bar below looks bad"* | **shipped 2026-08-18** | Two faults in the composition he picked the day before.<br><br>**The album was an ornament.** `marquee_edge` took `room * 0.34` capped at half a list measure, under a comment reading *"the object is no longer the subject of this page and does not want to be"* — which was the tell. Moving the title off the artwork meant the object stopped being the composition's *anchor*; it did not mean it should shrink. At 1920×1080 that drew a 290 px box, and a jewel case is drawn **in perspective inside** its box, so about **215 px of album reached the glass in a 1688×854 stage** with an 800 px cover sitting unused in the cache. The share is 0.58 now, capped at most of a list measure: close to 500 px at 1920 and a little over 700 at 2560.<br><br>**The existing test could not have caught it**, and that is the more useful half. It asserted `edge <= LIST_MEASURE * 0.5` — the cap being checked against itself — so a ceiling was watching a floor. It is stated against the **stage** now (`edge <= room * 0.72`), and a new test holds the share from below at three real window sizes. The one rule that did not move: never larger than the source's own pixels, swept over five sources and three stages.<br><br>**The two columns of type were twenty pixels apart.** The title block sits on `MARGIN` (1.5·HANG, 60) and the source band was padded with `HANG` (40) — close enough to read as a mistake rather than as a second deliberate margin. Both are `MARGIN`, pinned as an identity of the tokens so moving one moves both. |
| *"maybe we should add a few presets in a dropdown — that seems common"* / *"what does the suggest a pre-amp mean — a bit obscure?"* | **shipped 2026-08-18** | **Presets, and the objection that was half right.** This crate's note argued *"a list of named curves is somebody else's taste sold as a feature"*. That is true of **genre** presets — one called *Rock* claims to know what rock should sound like — and false as a general rule. *Quiet listening* claims that the ear loses the bottom and top of its range at low volume, which is measured; *Late night* claims deep bass travels through walls, which is a property of buildings. So the offered set is **situations, not genres**: Flat, More bass, More treble, Voices forward, Quiet listening, Late night — six, each shipping the sentence saying what it is *for*. A test refuses a genre name and refuses two presets with the same curve under different names.<br><br>Choosing one **takes its headroom with it**, because a preset that boosts is a preset that needs room and leaving the pre-amp where the last curve left it is how a listener gets a distorted first impression of a curve baz offered them. A hand-dragged curve reads as **Custom**; drag it back onto a preset's exact numbers and it is that preset again, because there is no hidden state to disagree with. The `Flat` button is gone — Flat is the first preset, so there is one control for *choose a curve* instead of one button for the only curve baz was willing to name. `theme::output_picker` became `theme::picker`: a style named after its first caller invites a near-copy for the second.<br><br>**And the pre-amp label was naming the machinery.** *Suggest a pre-amp* says there is a pre-amp and here is a value for it; it says nothing about the situation — you are boosting, a boost has to come from somewhere, this takes it from the whole signal so nothing clips. It reads **Make room**, and the line under the row now carries the numbers: *"The biggest lift here is +6 dB, and there is nowhere for it to go — loud parts can distort. Make room turns everything down by 6 so it fits."* That line has three states and only shows the warning when there is something to warn about. |
| *"Could you ensure that the UI of the equalizer is genuinely very good? This is not something that should be buried in the settings. It should be accessible potentially from anywhere maybe on the top bar… Sliders help show graphically. high the current of frequencies is biased."* | **shipped 2026-08-18** | Both halves of the ask were things the first version got wrong, and the second is the one worth reading.<br><br>**A door in the app bar.** The equaliser left Settings → Playback entirely. Its mark — three faders that disagree, so the door needs no word — stands beside the bell and the gear, resident in every place, because *from anywhere* was the requirement and a control you must navigate to is not from anywhere. The panel floats under it, Esc peels it, a press outside puts it away.<br><br>**Eleven faders over one datum**, ten bands and the pre-amp, the pre-amp behind a rule because it is headroom and not an eleventh frequency. Each band is read three ways deliberately — the handle's **height** is the curve at a glance, the fill repeats it as a **length**, and the **number** is there to match a band you set yesterday — so no reading rests on telling two hues apart.<br><br>**And the curve itself, drawn behind them** (`crate::response`). This is the part the handles could not do: two neighbouring bands at +6 dB do not make two +6 dB bumps, they overlap and make about +9 dB between them, and no arrangement of ten separate grips shows it. So the panel draws the **real magnitude response** — the same arithmetic the filters run, evaluated on the unit circle — and the faders stand on it. Both axes are *shared* rather than matched: up is `fader::y_of_db`, the fader's own mapping, so the curve passes through the handles instead of near them; across is log frequency, and since the ISO centres are octave-spaced, equal steps in log frequency are equal steps in column pitch. Band *i* peaks under band *i*'s fader by construction, and a test holds it.<br><br>**The pre-amp is deliberately not in the drawn curve.** It is a uniform offset — it moves every frequency equally and so contributes nothing to which frequencies are lifted over which others, which is the only thing the picture is for. Folding it in slides the whole shape off the handles that asked for it, so a listener who took the suggested pre-amp would see a curve touching none of their own controls and read the panel as broken. Pinned by a test, because it looks like an omission.<br><br>Drawn in quads rather than `canvas`, for `crate::contour`'s reason: `canvas` pulls a tessellation stack this project prices deliberately. Columns **tile** on rounded pixel boundaries instead of overlapping — the fill is alpha, and overlapping alpha composites twice into a picket fence — and the curve's edge is drawn from the previous sample's height to this one, so a single band at the limit is a line and not a ladder of rungs.<br><br>**And the test that was missing.** A live run crashed on the first frame that drew the new mark: `Glyph::Equalizer` had an `index()` arm and an outline and was absent from `Glyph::ALL`, so `handle` indexed 40 into a 40-element sheet — while all 29 icon tests passed, because every one of them started from `ALL` and the fault was a glyph that was not in it. `every_glyph_a_view_draws_is_in_the_sheet` starts from the other end: it reads the shipped views, finds every `Glyph::` they name, and asks the sheet for it. Confirmed by reintroducing the bug. |
| *"drag and drop yes (depending on the circumstances)"* | **shipped 2026-08-18 — the circumstance chosen and stated** | A file or folder dragged from a file manager onto a **running** baz is queued. The Setup screen already answered *where is your music*; once the library is open the question is different, and the queue is the one answer that cannot lose anything — nothing is scanned, nothing written, no root added, and the run you were listening to keeps playing with the drop behind it. A folder is walked for the audio it holds, in path order, so an album folder queues the album.<br><br>**Not an import**, deliberately: a gesture that quietly enlarged the library would be one drag away from a folder nobody meant to keep. Settings adds folders, with a dialog.<br><br>The extension list is the scanner's own `AUDIO_EXTENSIONS`, so a drop can never queue a file the shelf would refuse to list. The walk is bounded at 8 levels and 2 000 files, because a listener can drop `/` and an unbounded walk on a network share is a frozen window. A dropped file baz already knows keeps the metadata the scan read; one it has never seen queues anyway under its filename, because the engine plays paths.<br><br>**Not proven end to end, and the reason is the harness rather than the code.** A hand-written XDND source was built to synthesise a real drag (no file manager on the proof box). The X-level exchange completes — baz asks for the URI list and answers `XdndFinished` — but no `FileDropped` reaches the shell. The decisive check: the **same** synthesised drop produces nothing on the *Setup* screen either, where drop is a shipped and working feature. So winit is refusing the synthetic source, not baz refusing the drop. The walk, the filter, the ordering and the bounds are unit-tested; the message arm is two lines onto an event iced already delivers. Worth a real drag by hand. |
| *"equalizer yes"* / *"EQ should be something that can be enabled or disabled... without EQ it's just pure passthrough. with EQ it should be the highest quality possible?"* | **shipped 2026-08-18** | **Off is structural.** A disabled equaliser is not in the path at unity — it is not in the path: `Equalizer::is_active` sits beside the fader's `is_transparent` in the pump's short-circuit, so the ring still reaches the sink with no copy and no arithmetic. Enabled-but-flat answers the same way, because a cascade of ten unity filters would cost the bit-exact path for no change in the sound. Asserted end to end through the engine, sample for sample with no tolerance.<br><br>**On is `f64` state and coefficients** through a transposed-direct-form-II cascade, per channel, designed by the RBJ cookbook so the numbers mean what they mean in every other player. Ten ISO octave centres, ±12 dB, at a Q wide enough that a curve drawn across several bands stays a curve; a band above the stream's Nyquist is not built rather than built wrong.<br><br>**It does not normalise or limit.** A boost is a boost, and baz will not quietly rescale to hide a clip the listener asked for — what it offers instead is a stated pre-amp with `Suggest a pre-amp` deriving one from the curve. **`Flat` is the only preset**: a list of named curves is somebody else's taste sold as a feature.<br><br>~~**Stepper rows, not vertical faders.**~~ **Superseded the same day** by the row below, and it is worth keeping the mistake visible: that reasoning was about which control was cheapest to reach for, and the argument it dressed up — *the picture a fader row draws is the curve, and the curve belongs elsewhere* — conceded the whole point. A graphic equaliser **is** the picture. It shipped in Settings with ten stepper rows and was replaced within hours.<br><br>The protocol carries **centidecibels** rather than floats, for `SetVolume`'s reason: `Command` is `Eq` and crosses a boundary. A config with nine bands is flat rather than shifted by one frequency. |
| *"please fix the title on the now playing screen cos it just kinda floats there and looks out of place completely... also does not expand when the title is long. please put some design thinking and make me some mockups of options"* | **shipped 2026-08-18 — he chose C · Field from the drawn options** | **Third telling**, after *"the alignment and general styling… is poor"* and *"it still looks a bit poor… it really needs to pop"* — so by the repeated-ask rule the last two answers were the wrong shape, not under-explained. What they did: centred the three lines on one axis, then raised the title to its own display rung. What he is describing now is different and more fundamental — the block **floats**, unattached to anything, and the fitted line I added *caps* at the measure rather than growing into the room a long title needs. `views::fitted_line` truncates with an ellipsis at `edge`, which is the artwork's width, so a long title is cut on a surface with hundreds of empty pixels either side of it. **Designs before code, which is what he asked for.** Three directions were drawn against **all three foreground modes** — cover, jewel case and objectless — which is what the first attempt missed: a caption designed for a square has nothing to attach to when there is no square. He picked **C · Field**, objectless.

**What shipped**: one composition for all three modes. The object is small and top-right; the marquee is anchored to the place's own bottom-left margin, which is the one edge that cannot move when the artwork changes size, changes shape, or is not there. The artist line takes the record's own hue (`field::Field::inks`, the same colours the visualiser draws). The title wraps — **no ellipsis on this surface** — inside a measure far wider than the artwork ever was, stepping down a three-rung ladder (`SIZE_MARQUEE` 64 → `SIZE_DISPLAY` 40 → `SIZE_HERO` 28) as it grows.

**The two branches became one**, which is why the objectless state kept being designed last: it is now the same layout with the object absent.

**One argued decision reversed with it, on his choice**: the serif italic now sets a *track* title. `the_serif_is_the_work_titles_and_nothing_else` named `views/now_playing.rs` as explicitly excluded — *"a track's title, with the album under it as a fact about it"* — and that reading treated *work* as a synonym for *album*. On a surface that draws one thing, and where that thing is a track, the track is the work being labelled and the album line beneath is the fact about it, which is the placard's own order. The enumeration is widened and the reversal is recorded in it. |
| *"move thf flac/mp3 toggle out of the scroller as well"* | **shipped 2026-08-17** | The edition selector joins the sleeve and `Play album` in the held half (`page::Page::aside_held`), for the commitment's reason and one of its own: picking FLAC over MP3 **changes the rows beside it**, so scrolling away from it hides the cause of what you are looking at. `Details` stays in the scroller — it describes the page where the selector chooses it. The held head is now 424 px, which wants a window ~691 px tall before it clips; that is the cost, and it is stated in the same test that argues the rest of the split. |
| *"the now playing title area still looks a bit poor... it really needs to pop"* | **shipped 2026-08-17** | Second telling, so the size was the problem and not the arrangement. The title stood at `SIZE_HERO` **28** — the size the first-run *question* takes, and the size a record page gives an album title inside a dense two-column layout. Now playing is one work alone on a surface with nothing else asking to be read, so it gets its own rung: `SIZE_DISPLAY` **40** on a 44 px line box. **The fitting came with it, and had to**: the title was a raw `Wrapping::None` in a clipped container, so a long name stopped mid-glyph with nothing to say it continues — the same defect he reported in the bar — and at 40 px a title runs out of measure far sooner than at 28. It is `views::fitted_line` now, measured against the real `SemiBold` face. The artist eyebrow came up from `paper_faint` to `paper_dim` so the pair reads as a hierarchy rather than a voice and a whisper. The placard's reserved height moved with it (96 → 108) or the sleeve would have been sized against a shorter placard than the one drawn. |
| *"the visualisations seem to be all the same colour? some sort of weird green or something? it should be more dynamic and interesting"* | **shipped 2026-08-17** | They were literally one colour: `room.lamp`, one amber for every record, every band and every frame — and a thin amber over the green-tinted wash reads as neither. They take **the record's own three hues** now, the same three the background wash is built from, so a record's bars and its field are demonstrably one reading of one cover. The hue walks across the axis — bass to treble on the spectrum, oldest to newest on the waveform — so a field has a direction as well as a shape, and it changes with the record. Strength differs from the wash on purpose: the wash is a *ground* pinned near-neutral at chroma 0.024, and the bars are neither ground nor text, so they take chroma 0.11 at L 0.72 — **measured, like the wash's**: a binary search puts the largest chroma that clips no channel at that lightness at 0.113, and a test re-derives it. A record with no decoded cover falls back to the lamp, because inventing a colour for it would be the field module's cardinal sin. **The reading is still the height** — hue carries nothing, and a bar means what its length says in greyscale. |
...
| *"the bell icon is still weirdly skinny… backlog it"* | **shipped 2026-08-15 (`571c1a5`); the row is corrected 2026-08-18** | Item 48 measured the wrong thing and passed. It widened the bell to **0.78 of its box at the widest point**, and the widest point is the **rim** — a single hairline at the bottom of the glyph. What a reader sees as the bell's weight is its **dome**, which item 40 drew at 0.30 and item 48's uniform 1.147 scale about the vertical axis carried to about **0.34** — against the gear's disc at 0.84 in the identical box, so the mark is still less than half its neighbour's mass and still reads skinny, exactly as he says. The test is complicit: `the_bell_is_as_wide_as_the_cluster_it_stands_in` takes the **maximum** run width and allows 0.07 of slack under the narrowest neighbour, so a wide rim under a narrow dome passes it. **The fix was the dome**, with the flare following it — a 0.60 waist flaring to the same 0.78 rim — and a test measuring the body's *median* run rather than its maximum.<br><br>**And it happened again three days later, which is the part worth keeping.** `Glyph::Equalizer` was added on 2026-08-18 and laid **0.179** of its box in ink beside a gear at 0.324 and this bell at 0.342 — 52% of its heaviest neighbour, the same reading he had called skinny twice. Neither bell fix could have caught it: both were bell-shaped, one measuring the widest scanline and the next the median run, and neither said anything to a mark that had not been drawn yet. A glyph is read by its **mass**, which is neither of those — a wide outline can be a hairline and a tall one can be a spike. So `a_mark_in_the_app_bar_carries_its_neighbours_weight` measures alpha-weighted ink over the **whole cluster** and holds the lightest above 0.75 of the heaviest, plus an absolute floor so a uniformly faint cluster cannot pass by agreeing with itself. The equaliser's rails went 0.05 → 0.10 and its handles 0.11 → 0.21 tall, putting it at 0.31. Confirmed by restoring the old outline and watching the test name the 52%. |
...
| *"figure out why we are using so much memory… I see 1.8GB"*<br>*(recorded with a first diagnosis, not yet fixed)* | **part fixed and re-measured 2026-08-18; the peak is still open** | **The arithmetic points almost entirely at the Vibe model sessions, and it lands on his figure.** `baz-vibe`'s `semantic::Model::load` opens **both** ONNX towers — `text_model_quantized.onnx` **126 MB** and `audio_model_quantized.onnx` **34 MB** — and the model is a `thread_local!`, created lazily per analysis worker and **never released**. The default worker count is `DEFAULT_VIBE_WORKERS` **8** (`MAX_VIBE_WORKERS` 16), so a scan materialises up to `8 × 160 MB` = **1.28 GB of weights** before ONNX Runtime's own per-session arenas are counted — and **every worker loads the text tower it never uses**: a worker only ever calls `audio()`, while the prompt is embedded once, on whichever thread runs the request. Measured against that: three idle baz processes on a 206-track fixture library, no analysis run, sat at **~260 MB** RSS each, which is the ordinary baseline (artwork's stated 170 MiB budget, the index, wgpu's driver mappings). 260 MB + 1.28 GB is 1.5 GB before arenas, and 1.8 GB is exactly where that lands. **How to tell it apart in ten seconds**: Settings → Debug reports this process's own RSS (item 39). If it reads ~250–300 MB before a Vibe scan and >1 GB after, this is it; if it is already high before any scan, the suspect is artwork or the renderer instead and the entry is wrong. *The fixes, cheapest first: load each tower only where it is used (8 workers × audio alone is 272 MB, not 1.28 GB); release the sessions when a scan ends rather than holding them for the process's life; cap workers against memory rather than against cores; and price ORT's arena/memory-pattern options with a measurement rather than a guess.*<br><br>**2026-08-18 — the floor was not what this entry said it was.** The first two fixes shipped (each tower opened where it is used; one shared text tower behind a mutex). The third — *release the sessions when a scan ends* — is now done, and **it returned nothing**: `released the text tower: 826 MiB -> 826 MiB`. `free` handed the arena back to glibc, which kept it. The floor was *retained*, not live, which is the opposite of what this entry assumed and could only be told apart by asking the process what it held either side of the drop.<br><br>**`malloc_trim` is what returns it** — one FFI call on the same path, **106–160 MiB every run** across five paired readings. A second trim after the workers retire was tried and dropped (`762 -> 762`): ONNX Runtime's per-session arena is not on glibc's free lists.<br><br>**And the harness had stopped reaching the thing it measures.** Three UI moves had happened under a script that clicks blind coordinates; one repaired run reported a 355 MiB compose peak against a recorded 1 363, and the run after that looked right at 1 751 until a screenshot showed the typed request had gone into the app-bar search while the number came from the analysis behind it. A harness that cannot reach its subject does not fail — it reports a different number in the same shape. It now checks it arrived, saves screenshots, and prints `VOID` on an idle-shaped curve.<br><br>**Honestly: the peak is still 1.6–1.9 GiB and his 1.8 GB is not fixed.** This returns part of the *floor*, which is what baz holds for the rest of the session after one compose. The peak's fix is the one already named — one shared audio session behind a mutex, roughly 400 MiB, trading wall-clock — and it needs a time measurement first. Also learned: the floor swings 160 MiB run to run, so **no claim here may rest on comparing two runs**; the paired within-process reading is the only sound one. |

...
| Ask | State | Where |
|---|---|---|
| *"I'd like to also backlog a resource usage feature in the app to show how much RAM/CPU it is using in the debug menu"* | **backlogged 2026-08-14** | Settings → Debug (item 23's section) gains a live readout of the running process's own RAM and CPU usage — resident set size and a CPU figure, sampled only while that place is visible and labelled so it cannot be mistaken for a listener-facing health fact. This gives item 37's memory-budget decision a measurement surface inside the app rather than only in the developer console. *Needs later: priority; whether it joins the 256-line ring or draws fresh while Debug is open; per-platform source of truth (`/proc/self/status` and friends); and a deterministic test. See WORK.md's brief below.* |

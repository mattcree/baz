...
| *"we should ensure the density options are available on all pages..."* / *"4 levels makes sense to me"* | **building** | the control is drawn in the index rail's foot, which exists only on the wall — while Home already lays its tiles at a density it offers no way to change. `Density::ALL` is three steps and its own doc anticipates a fourth. The open question is what density *means* on a page of rows rather than tiles: it either scales them honestly or the control must not appear there inert |
| *"I guess we need to add playlists into their own section under library"* | **building** | the lane's `RECENT` already holds every playlist mixed with the last records, so this is a split rather than an addition — and it reverses his own *"recent albums and playlists mixed based on some order"* from when the lane was designed, so ADR-0030 is amended rather than rewritten |

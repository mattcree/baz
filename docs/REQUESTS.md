...
| *"the album count in the bottom bar when in shuffle mode is weird... way too many albums shown"* | **logged, undiagnosed** | suspected: the count is drawn from the traversal's bag rather than the run, so it counts what *could* come rather than what is in hand |

...
impl App {
    fn new(started: Instant, cli_dir: Option<PathBuf>) -> (Self, Task<Message>) {
        ...
        let (screen, task) = match dir {
            ...
            Some(dir) => match Shelf::open(dir, group_key, density) {
    ...
    fn update(&mut self, message: Message) -> Task<Message> {
        ...
        if let Some(task) = self.update_modified_input(&message) {
            return task;
        }
    ...
    fn submit_setup(&mut self) -> Task<Message> {
        ...
        match Shelf::open(dir, self.group_key, self.density) {
            Ok((state, task)) => {
...
impl Shelf {
    ...
    fn update(&mut self, message: Message) -> Task<Message> {
        match message {
            ...
            //
            // The text is *appended*, never assigned: a listener who has
            // already typed and clicked away mid-query continues it rather than
            // restarting it, which is what the well would do if the caret were
            // still in it. In practice the append runs once and then the well
            // has focus, so this is the empty-query case almost every time.

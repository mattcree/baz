...
| `IBMPlexMono-Regular.ttf` | IBM Plex Mono Regular | 2.005 | 173 052 | `7c6fbddca4b700be918f5f6183d9bd4464fa427fe435f0b480d77fe2bb8c5a43` |
...
`“” ‘’`) produces 335 556 bytes across the five faces, saving ~666 KB. It was
...
## Which faces, and why these five

`docs/design/02-visual-language.md` §2.2.1–§2.2.3 asks for exactly:

- **Sans Regular / Medium / SemiBold** — the interface voice. All three exist as
  real drawn faces, so nothing is synthesised and nothing falls back.
- **Mono Regular** — every figure that changes in place (§2.2.4). iced 0.13 has
  no OpenType feature control, so there is no `tnum`; the monospace *is* baz's
  tabular figures. Plex Mono shares Plex Sans's x-height, stem weight and
  terminal treatment, so `2022 · 8 tracks · 47:21` reads as one typeface with
  two settings instead of two unrelated voices in one line.
- **Serif SemiBold** — the album title and the first-run question, and nothing
  else (§2.2.3).

No italics: baz sets none.

## The metrics this asset is accountable for

Plex Mono advances **600/1000 em for every glyph** — 20% wider than the 0.5-em
figure `theme.rs`'s reserved-slot assertions used to guess with. Every
fixed-width slot in the pixel-stable bottom bar was sized against the old face,
so `src/font.rs`'s tests parse these very bytes (`head`, `hhea`, `hmtx`, `cmap`)
and measure the real advance width of each worst-case string against the token
reserving it. Changing a file here without re-running `cargo test -p baz` is how
a duration silently clips.
